...
#change_contrasts_sum
#Whether to change contrasts if models are run under type_of_stats==freq_glm_glmer (T or F)
#If TRUE, contrasts will be changed to options(contrasts = c("contr.sum", "contr.poly"))
#Defaults to TRUE
...
plot_proportion_stats<-function(input_data,
                                ...
                                change_contrasts_sum=T, ## matters only if type_of_stats=="freq_glm_glmer" and either stats_for_1sub or stats_for_2sub ==T
  ...
  #change_contrasts_sum
  if(type_of_stats!="freq_binom"&(stats_for_1sub|stats_for_2sub)){
    if(length(change_contrasts_sum)!=1){
      stop("change_contrasts_sum has to be a boolean vector of length 1 (either TRUE or FALSE).")
    }
    if(!is.vector(change_contrasts_sum)|is.list(change_contrasts_sum)){
      stop("change_contrasts_sum has to be a boolean vector of length 1 (either TRUE or FALSE).")
    }
    if(!change_contrasts_sum%in%0:1){
      stop("change_contrasts_sum has to be boolean (T or F).")
    }
  }
  ...
  if(!only_stats){
    ...
    if(stats_for_1sub|stats_for_2sub){
      ...
      if(length(suppress_est1_if2)!=1){
        stop("suppress_est1_if2 has to be a boolean vector of length 1.")
      }
      if(!is.vector(suppress_est1_if2)|is.list(suppress_est1_if2)){
        stop("suppress_est1_if2 has to be a boolean vector of length 1.")
      }
      if(sum(!suppress_est1_if2%in%0:1)>0){
        stop("suppress_est1_if2 has to be boolean (T or F).")
      }
  ...
  glm_glmer<-function(data1=NULL,
                      ...
                      change_contrasts_sum1=change_contrasts_sum,
